...
function shouldTranslateToOpenRouterChat(
  ...
  // When OPENROUTER_MODEL is configured, route plain model IDs (no slash) through
  // /v1/chat/completions so ZDR provider routing is respected.
  if (secrets.OPENROUTER_MODEL && !model.includes('/')) return true;
...
function buildOpenRouterChatBody(body: JsonObject, secrets?: Record<string, string>): JsonObject {
  const model = (secrets?.OPENROUTER_MODEL || (body.model as string));
  const providerOrder = secrets?.OPENROUTER_PROVIDER_ORDER;
  const provider = providerOrder ? null : getProviderFromModel(model);
  ...
  return compactObject({
    ...
    provider: providerOrder
      ? { order: providerOrder.split(',').map((s) => s.trim()), allow_fallbacks: false }
      : provider
...
async function handleTranslatedOpenRouterRequest(
  ...
  const translatedBody = Buffer.from(
    JSON.stringify(buildOpenRouterChatBody(bodyJson, secrets)),
    'utf-8',
  );
